   ...
   switch (prefix) {
      ...
      case 'info':
       ...
       case 'nulis':
           nulis(value)
               .then(data => {
                   client.sendMessage(id, '[ESPERE UM POUCO AMIGO...', MessageType.text)
       ...
       case 'ytmp4':
           ytdl('mp4', value)
               .then(data => {
                   const { judul, size, hasil: link } = data
                   let hasil = `OLA AQUI ESTA O LINK DE DOWNLOAD DA MSC\nCLIQUE NO LINK ABAIXO:\nMUSICA: ${judul}\n\nTamanho Do audio: ${size}\n\nLink: ${link}`
                   client.sendMessage(id, '[ESPERE UM POUCO AMGO...', MessageType.text)
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'covid':
           corona()
               .then(data => {
                   ...
                   let hasil = `📌ÚLTIMOS DADOS DO DISTRITO COVID-19 DO BRAZIL\n\n📍Positivo ==> ${positif} \n📍Curado ==> ${sembuh} \n📍Morreu ==> ${meninggal}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
   ...
   case 'bitly':
    ...
    case 'bucin':
     ...
     case 'cooltext':
           ...
           ttp()
               .then(buffer => {
                   client.sendMessage(id, '[ESPERE UM POUCO AMIGO, O COMPUTADOR DO TEFINHO È FRACO KKKKKKK...', MessageType.text)
                   client.sendMessage(id, buffer, MessageType.image)
               })
               .catch(err => {
                   console.log(err)
           ...
           nekonime()
               .then(buffer => {
                   client.sendMessage(id, '[ESPERE UM POUCOA MIGO, O COMPUTADOR DO TEFINHO È FRACO KKKKKKK', MessageType.text)
                   client.sendMessage(id, buffer, MessageType.image)
               })
               .catch(err => {
                   console.log(err)
